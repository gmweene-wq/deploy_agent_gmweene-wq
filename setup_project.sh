...
cleanup() {
    ...
    if [ -d "attendance_tracker_${input}" ]; then
        # Create archive directory and copy the incomplete project
        cp -r "attendance_tracker_${input}" "attendance_tracker_${input}_archive"
        echo "Incomplete project archived as: attendance_tracker_${input}_archive/"
...
# Set up signal trap for SIGINT (Ctrl+C) - now after folder creation
trap cleanup SIGINT
